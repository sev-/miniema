/*
 * $Id: display.c,v 1.3 1994/08/15 20:42:11 sev Exp $
 * 
 * ----------------------------------------------------------
 * 
 * $Log: display.c,v $
 ...
 * Indented
 * Revision 1.2  1994/06/24  17:22:21  sev Added updoneline
 * function (for screen-piece refershing)
 * 
...
WINDOW *wp;			       /* window to update lines in */
{
  ...
  register int sline;		       /* physical screen line to update */
  register int i;
  register int nlines;		       /* number of lines in the current
				        * window */
...
WINDOW *wp;			       /* window to update modeline for */
{
  ...
  register int lchar;		       /* character to draw line in buffer
				        * with */
